...
The Workflow Engine subscribes to events from agents and the API layer rather than being called directly.

### Events Subscribed

| Event | Action |
|-------|--------|
| `AgentTaskCompleted` | Process artifacts, check phase completion |
| `AgentTaskFailed` | Handle failure, create escalation if needed |
| `ArtifactApproved` | Update artifact status, check phase completion |
| `ArtifactRejected` | Mark for re-work, notify agent |
| `EscalationResolved` | Unblock workflow, resume agent |

### Event Handler Pattern

```go
func NewWorkflowEventHandler(svc *WorkflowService, bus EventBus) {
    bus.Subscribe([]string{
        ...
        case *AgentTaskCompletedEvent:
            return svc.handleTaskCompletion(ctx, e)
        case *AgentTaskFailedEvent:
            return svc.handleTaskFailure(ctx, e)
        case *ArtifactApprovedEvent:
            return svc.handleArtifactApproval(ctx, e)
        case *ArtifactRejectedEvent:
            return svc.handleArtifactRejection(ctx, e)
        }
        return nil
    })
}
...
| Event | When | Payload |
|-------|------|---------|
| `PhaseStarted` | New phase begins | workflowId, phase |
| `PhaseCompleted` | All artifacts approved | workflowId, phase |
| `EscalationCreated` | Constraint violation | escalationId, constraintId |
| `WorkflowCompleted` | All phases done | workflowId, projectId |
