...
graph TB
    ...
    ReqAgent -->|events| EventBus
    ArchAgent -->|events| EventBus
    CodeAgent -->|events| EventBus
    SecAgent -->|events| EventBus
    EventBus --> Workflow
    EventBus --> WS
```
...
| Event Bus | Decoupled communication between agents, workflow, and clients |
...
### REST/WebSocket Consistency Model

The system uses a hybrid approach for state synchronization:

| Operation | Primary Channel | Consistency |
|-----------|-----------------|-------------|
| Read operations | REST API | Strong (latest from DB) |
| Write operations | REST API | Strong (DB transaction) |
| Real-time updates | WebSocket | Eventually consistent |
| Optimistic UI | Client-side | Reconcile on server response |

### Synchronization Protocol

1. **Client writes via REST**: All mutations go through REST API
2. **Server persists to Firestore**: Transactional writes
3. **Event Bus broadcasts**: After successful write
4. **WebSocket pushes to clients**: All project members notified
5. **Client reconciles**: Compare local state with server update

### Conflict Resolution

- **Server authoritative**: On conflict, server state wins
- **Version vectors**: Each entity has version for optimistic locking
- **Rollback on conflict**: Client optimistic updates rolled back if server rejects

See [Edge Cases: Concurrency](./edge-cases/concurrency.md) for detailed scenarios.
...
- [LLM Resilience](./llm-resilience.md) - Provider failure handling
