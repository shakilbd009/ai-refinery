...
type User struct {
    ...
    MFAEnabled    bool      `firestore:"mfaEnabled"`
    CreatedAt     time.Time `firestore:"createdAt"`
...
type RefreshToken struct {
    ...
    TokenHash   string    `firestore:"tokenHash"` // SHA-256 of token
    ExpiresAt   time.Time `firestore:"expiresAt"`
    CreatedAt   time.Time `firestore:"createdAt"`
    RotatedFrom *string   `firestore:"rotatedFrom"` // Previous token ID
}

type MFAFactor struct {
    ID         string    `firestore:"id"`
    UserID     string    `firestore:"userId"`
    Type       string    `firestore:"type"` // totp, sms
    Secret     string    `firestore:"secret"` // Encrypted TOTP secret or phone
...
type RecoveryCode struct {
    ID        string    `firestore:"id"`
    UserID    string    `firestore:"userId"`
    CodeHash  string    `firestore:"codeHash"` // SHA-256 of code
