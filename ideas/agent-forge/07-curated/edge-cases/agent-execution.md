...
### Circuit Opens During Task

**Trigger:** LLM failures trigger circuit breaker mid-task

**Impact:** Task cannot complete

**Mitigation:**
- Checkpoint task state before LLM calls
- Queue task for retry when circuit closes
- Notify user: "AI service temporarily unavailable"
- Show estimated recovery time if known

### Circuit Half-Open Probe Fails

**Trigger:** Probe request fails during recovery

**Impact:** Circuit stays open longer

**Mitigation:**
- Use lightweight probe requests
- Exponential backoff between probes
- Manual override available for admins
- Alert if circuit open > 10 minutes

### Cascading Circuit Opens

**Trigger:** Multiple agent types experience failures simultaneously

**Impact:** All agent work halts

**Mitigation:**
- Per-provider circuit breakers (not global)
- Rate limiting prevents thundering herd
- Graceful degradation: queue non-urgent work
- Status page shows provider health

### Token Budget Exhaustion

**Trigger:** Organization hits token limit

**Impact:** All LLM requests rejected

**Mitigation:**
- Warn at 80% of budget
- Queue non-critical requests
- Admin can increase budget
- Show clear "budget exceeded" message
