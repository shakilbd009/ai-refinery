...
AgentForge's security design covers five main areas:

1. **Authentication** - JWT tokens, MFA, session management
2. **User & Org Permissions** - Multi-tenancy, roles, project access
3. **Input Validation** - Rate limiting, prompt injection defense
4. **Agent Sandboxing** - Execution isolation, data boundaries, resource limits
5. **Security Review** - Code vulnerability scanning before completion
...
- [Authentication](./authentication.md) - JWT, MFA, session management
- [Secrets Management](./secrets-management.md) - GCP Secret Manager integration
- [Input Validation](./input-validation.md) - Rate limiting, prompt injection defense
