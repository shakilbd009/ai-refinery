...
// GetGuidelinesForAgents loads guidelines for multiple agent types in a single query
func (s *KnowledgeService) GetGuidelinesForAgents(ctx context.Context, orgID string, agentTypes []domain.AgentType) (map[domain.AgentType][]*domain.Guideline, error) {
	return s.repo.GetGuidelinesForAgents(ctx, orgID, agentTypes)
}

// GetConstraintsForAgents loads constraints for multiple agent types in a single query
func (s *KnowledgeService) GetConstraintsForAgents(ctx context.Context, orgID string, agentTypes []domain.AgentType) (map[domain.AgentType][]*domain.Constraint, error) {
	return s.repo.GetConstraintsForAgents(ctx, orgID, agentTypes)
}

// BatchGetKnowledge loads all knowledge types for a single agent in parallel
func (s *KnowledgeService) BatchGetKnowledge(ctx context.Context, orgID string, agentType domain.AgentType) (*AgentKnowledge, error) {
	var (
	...
	go func() {
		var err error
		guidelines, err = s.repo.ListGuidelines(ctx, orgID, agentType)
		errs <- err
	}()
	go func() {
		var err error
		constraints, err = s.repo.ListConstraints(ctx, orgID, agentType)
		errs <- err
	}()
	go func() {
		var err error
		templates, err = s.repo.ListTemplates(ctx, orgID, agentType)
		errs <- err
	}()
	go func() {
		var err error
		examples, err = s.repo.ListExamples(ctx, orgID, agentType)
		errs <- err
	}()
...
// AgentKnowledge bundles all knowledge for an agent type
